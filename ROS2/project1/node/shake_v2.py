...
# 두산 api 내부 전역 변수 설정(없으면 모든 move 함수 먹통)
import DR_init
DR_init.__dsr__id   = ROBOT_ID
DR_init.__dsr__model = ROBOT_MODEL

# 이건 왜 쓰지?
logger = get_logger('shaking_logger')
...
def main(args=None): 
        ...
        node = rclpy.create_node('shaking', namespace=ROBOT_ID) # 노드 이름: shaking, 네임스페이스: /dsr01/shaking
        DR_init.__dsr__node = node

        try:
                from DSR_ROBOT2 import movej, move_periodic
